"use client";

import React from 'react';
import { useAuth } from '@/context/AuthContext';
import { LogIn } from 'lucide-react';

export default function LoginPage() {
  const { login, error, clearError } = useAuth();

  return (
    <div className="flex flex-col items-center justify-center min-h-[80vh] px-4 animate-fade-in">
      <div className="w-full max-w-sm card-depth p-8 flex flex-col items-center gap-8 bg-bg-secondary">
        <div className="flex flex-col items-center gap-2">
          <div className="bg-accent text-white p-4 rounded-3xl shadow-lg animate-pulse-ring">
            <LogIn size={40} />
          </div>
          <h1 className="text-2xl font-black mt-4 tracking-tight uppercase">Gym Logger</h1>
          <p className="text-text-tertiary text-sm text-center">
            Track your progress, build your body, and maintain the aesthetic.
          </p>
        </div>

        {error && (
          <div className="w-full p-4 bg-red-500/10 border border-red-500/20 rounded-2xl flex flex-col gap-2">
            <p className="text-red-500 text-xs font-bold uppercase tracking-wider text-center">
              Login failed
            </p>
            <p className="text-text-primary text-sm text-center">
              {error}
            </p>
            <button 
              onClick={clearError}
              className="text-[10px] text-text-tertiary uppercase font-black hover:text-text-primary transition-colors"
            >
              Dismiss
            </button>
          </div>
        )}

        <button 
          onClick={login}
          className="w-full bg-white text-black border border-border-color py-4 rounded-2xl flex items-center justify-center gap-3 font-bold hover:bg-zinc-50 transition-all active:scale-95 shadow-md"
        >
          <svg width="20" height="20" viewBox="0 0 48 48">
            <path fill="#EA4335" d="M24 9.5c3.54 0 6.71 1.22 9.21 3.6l6.85-6.85C35.9 2.38 30.47 0 24 0 14.62 0 6.51 5.38 2.56 13.22l7.98 6.19C12.43 13.72 17.74 9.5 24 9.5z"/>
            <path fill="#4285F4" d="M46.98 24.55c0-1.57-.15-3.09-.38-4.55H24v9.02h12.94c-.58 2.96-2.26 5.48-4.78 7.18l7.73 6c4.51-4.18 7.09-10.36 7.09-17.65z"/>
            <path fill="#FBBC05" d="M10.53 28.59c-.48-1.45-.76-2.99-.76-4.59s.27-3.14.76-4.59l-7.98-6.19C.92 16.46 0 20.12 0 24s.92 7.54 2.56 10.78l7.97-6.19z"/>
            <path fill="#34A853" d="M24 48c6.48 0 11.93-2.13 15.89-5.81l-7.73-6c-2.15 1.45-4.92 2.3-8.16 2.3-6.26 0-11.57-4.22-13.47-9.91l-7.98 6.19C6.51 42.62 14.62 48 24 48z"/>
          </svg>
          Sign in with Google
        </button>
        
        <p className="text-[10px] text-text-tertiary uppercase tracking-widest font-bold">
          Powered by Next.js & Firebase
        </p>
      </div>
    </div>
  );
}
